...
from .services.workflow_status import apply_workflow_status
...
def on_startup():
    ...
    ensure_banktxn_columns()
    ensure_invoice_payment_match_columns()
...
def _classified_banktxn_ids(session: Session) -> set[int]:
    rows = session.exec(
        select(CashflowActual.banktxn_id).where(CashflowActual.banktxn_id != None, CashflowActual.invoice_id == None)
    ).all()
...
def releves_page(request: Request, session: Session = Depends(get_session)):
    ...
    try:
        accounts = session.exec(select(CompanyBankAccount)).all()
        accounts = [a for a in accounts if getattr(a, "is_active", True) in (True, 1, None)]
        accounts = sorted(
            accounts,
            key=lambda a: (
                (getattr(a, "bank_name", None) or "").upper(),
                (getattr(a, "account_no", None) or "").upper(),
            ),
        )
    except Exception:
        has_error = True
...
@app.post("/releves/{txn_id}/to_facture")
def releve_to_facture(
    txn_id: int,
    request: Request,
    session: Session = Depends(get_session),
):
...
def matching_manual(
    ...
    if amt is None or float(amt) <= 0:
        # priorité au solde facture, borné au mouvement
        amt = min(inv_amt if inv_amt > 0 else txn_amt, txn_amt)

    if float(amt or 0.0) <= 0:
        return _back("invalid_amount")
    if float(amt or 0.0) - float(inv_amt or 0.0) > 1e-9:
        return _back("amount_exceeds_invoice")
    if float(amt or 0.0) - float(txn_amt or 0.0) > 1e-9:
        return _back("amount_exceeds_txn")
    ...
    try:
        ...
        txn.processing_status = "RAPPROCHEE"
        session.add(txn)
        session.commit()
...
def matching_page(request: Request, session: Session = Depends(get_session)):
    ...
    matched_txn_ids = {int(x) for x in session.exec(select(InvoicePaymentMatch.banktxn_id)).all() if x is not None}
    classified_ids = _classified_banktxn_ids(session)
    ...
    unreconciled = [t for t in txns_all if int(t.id or 0) not in matched_txn_ids]
    txns = [t for t in unreconciled if float(t.debit or 0.0) > 0 and (t.processing_status or "").strip().upper() == "FACTURE" and int(t.id or 0) not in classified_ids][:500]
    qualified_rows = [t for t in unreconciled if (t.processing_status or "").strip().upper() == "HORS_FACTURE" and int(t.id or 0) not in matched_txn_ids][:250]
    ...
    txns_for_scan = txns[:80] if auto else ([focus_txn] if focus_txn else txns[:40])
    for t in [x for x in txns_for_scan if x is not None]:
        try:
            candidates = _candidate_invoices_for_txn(session, t, invs)
            suggestions_map[int(t.id)] = _suggest_for_txn(session, t, candidates, limit=8 if focus_txn else 5)
        except Exception:
            suggestions_map[int(t.id)] = []

    automation = _build_automation_snapshot(session)
    high_conf_matches = automation["high_conf_matches"][:20]

    processing_map = _banktxn_processing_map(session, qualified_rows) if qualified_rows else {}
...
def releves_classify_txn(
    ...
    txn = session.get(BankTxn, txn_id)
    if not txn:
        return RedirectResponse(f"{back}?msg=bad_txn", status_code=303)
...
@app.post("/matching/{txn_id}/unclassify")
def matching_unclassify_txn(
    txn_id: int,
    request: Request,
    session: Session = Depends(get_session),
):
    ...
    txn = session.get(BankTxn, txn_id)
    if not txn:
        return RedirectResponse("/matching?msg=bad_txn", status_code=303)
    ...
    rows = session.exec(
        select(CashflowActual).where(
            CashflowActual.banktxn_id == txn_id,
            CashflowActual.invoice_id == None,
        )
    ).all()
    for row in rows:
        session.delete(row)
    ...
    session.add(txn)
    session.commit()
    return RedirectResponse(f"/matching?msg=unclass_ok&txn={txn_id}", status_code=303)
...
def analytics_allocations_add(
    ...
    if amt <= 0:
        return RedirectResponse("/analytics/allocations?msg=invalid_amount", status_code=303)
    remaining = _invoice_amount_for_alloc(inv) - _alloc_sum(session, int(inv.id), "ANALYTIC")
    if amt > remaining + 0.01:
        return RedirectResponse("/analytics/allocations?msg=exceeds_remaining", status_code=303)
    line = AllocationLine(
...
def budget_allocations_add(
    ...
    amt = _to_float(amount) or 0.0
    if amt <= 0:
        return RedirectResponse("/budget/allocations?msg=invalid_amount", status_code=303)
    remaining = _invoice_amount_for_alloc(inv) - _alloc_sum(session, int(inv.id), "BUDGET")
    if amt > remaining + 0.01:
        return RedirectResponse("/budget/allocations?msg=exceeds_remaining", status_code=303)
    line = AllocationLine(
        invoice_id=invoice_id,
        kind="BUDGET",
        post_id=post_id,
...
async def factures_scan(
    ...
    if len(files) == 1:
        ...
        if doc_type in ("FACTURE", "BR") and (inv.invoice_date or inv.service_date):
            compute_pay_delay_fields(inv)
        apply_workflow_status(session, inv)
        session.add(inv)
